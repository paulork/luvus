...
- **devin**: adds one `SessionStart` entry to Devin's user config
  ...
  `SessionStart` and keeps other tools' hooks and the config's file
  permissions intact. A config with comments, or one that is a symlink, is
  refused instead of rewritten, and files selected with `devin --config` are
  not managed.
